<!-- 
:convenience_store: Bazar Julio Cesar
Commit Message:  Arrow Fix
-->

# 🏪 Bazar Julio Cesar Imperador

Landing Page institucional desenvolvida para o **Bazar Júlio Cesar**, referência em variedades, brinquedos e utilidades em Caxias do Sul.

## 🚀 Objetivo
Criar uma vitrine digital que apresente a diversidade de produtos da loja e simplifique o canal de atendimento, direcionando o tráfego online para o WhatsApp e para a loja física.

## ✨ Funcionalidades
- **Vitrine de Categorias:** Destaque para Ferramentas, Jardim, Doces, Material Escolar e Eletrônicos.
- **Hub de Redes Sociais:** Conexão direta com Instagram, Facebook e TikTok.
- **Contato Inteligente:** Botões de ação para WhatsApp e discagem direta.

### Principais recursos:

- 🎨 **Design:** Criado com ajuda da Inteligência Artificial da Google o Stitch;
- 🧩 **Organização:** Estrutura limpa e escalável;
- 🔗 **Dados:** Não possui consumo de dados;
- 📱 **Layout Responsivo:** Compatível com dispositivos móveis;
- 🏡 **Hospedagem:** GitHub e Vercel;

## ❤️ Ajude um Desenvolvedor

Se você gostou do site e de entrar rapidamente seu jogo retro favorito, considere ajudar o desenvolvedor com um valor simbólico.

If you liked the site and quickly jumping into your favorite retro game, consider helping the developer with a symbolic amount.

  <a href="https://github.com/sponsors/igorlazzaretti">
    <img src="https://img.shields.io/badge/sponsor-30363D?style=for-the-badge&logo=GitHub-Sponsors&logoColor=#white" alt="Sponsor on GitHub" />
  </a>

## 👨‍💻 Desenvolvedor

**Igor Dossin Lazzaretti**
🔗 [LinkedIn](https://www.linkedin.com/in/igorlazzaretti/)
🌐 [Portfólio](https://igorlazzaretti.com/)
💻 [Projetos GitHub](https://github.com/igorlazzaretti?tab=repositories)
